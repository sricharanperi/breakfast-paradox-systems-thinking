...
```mermaid
flowchart TB
  subgraph Core["Core — daily, direct interaction"]
    ST((Students))
  ...
  ST -.->|rates meals; reads unconfirmed| MC
  MC -->|sets menu, policy| MO
  MC -.->|no confirmed relationship| ACAD
  MO -.->|no confirmed relationship| ACAD
  WD -.->|no confirmed relationship| ACAD
  MO -->|four-day order lead| VSI & VNI & VJ
  ...
  SVC -->|serves| ST
  FAC -.->|renovation| VSI
  PIT -.->|interface for| ST
  PIT -.->|forecast to| MO
  ST -.->|peer influence, internal| ST
...
```mermaid
flowchart TB
  subgraph RingOuter["Outer ring — environmental and boundary"]
    direction TB
    VIN[Vindhya / juice canteens]
    RID[Delivery riders]
    ACAD["Academic administration\nsleeping stakeholder"]
